...
const Batchdetails = () => {
  ...
  // Check if the user has already joined the batch
  const hasJoinedBatch = (batch) => {
    return batch.join.some((join) => join.userid === user?.result?._id);
  };

  const handlepostans = (e, joinlength, maxStudents) => {
    e.preventDefault();
    if (user === null) {
      alert("Login or Signup as a student to join a batch.");
      navigate("/Auth");
    } else {
      if (!termsChecked) {
        alert("Please confirm all the terms and conditions before proceeding.");
        throw new Error("Terms and conditions not confirmed");
      } else {
        ...
        if (hasJoinedBatch(batch)) {
          ...
          setTermsChecked(false); // Reset the checkbox after submission
        }
      }
    }
  };
  ...
  const handleupvote = () => {
    if (user === null) {
      alert("Login or Signup as a student to vote for a batch.");
      navigate("/Auth");
    } else {
      dispatch(votebatch(id, "upvote"));
    }
  };
  ...
  const handledownvote = () => {
    if (user === null) {
      ...
      dispatch(votebatch(id, "downvote"));
    }
  };
